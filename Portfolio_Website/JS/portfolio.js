...
function showSlides(n) {
    ...
    var dots = document.getElementsByClassName("dot"); // This takes all elements with the class name "dots" and stores them in the array "dots"
    ...
    if (n < 1) {slideIndex = slides.length}; //If n (the number passed intor the function) is less than 1, the slideIndex is set to the length of the array "slides" 
    for (i = 0; i < slides.length; i++) {
        slides[i].style.display = "none"; //This for loop takes each item in the array "slides" and sets teh display to none
    }
    ...
    dots[slideIndex - 1].className += " active"; // This addes the active styling to the dot associated with the image
}

//This code will create clos the contact form when the user clicks off of it
